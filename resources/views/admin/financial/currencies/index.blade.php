@extends('admin.layouts.grid')
@section('section', translate('Financial'))
@section('title', translate('Currencies'))
@section('create', route('admin.financial.currencies.create'))
@section('container', 'container-max-lg')
@section('content')
    <div class="card">
        <div class="card-header p-3 border-bottom-small">
            <form class="multiple-select-search-form" action="{{ request()->url() }}" method="GET">
                <div class="row g-3">
                    <div class="col-12 col-lg-10">
                        <input type="text" name="search" class="form-control" placeholder="{{ translate('Search...') }}"
                            value="{{ request()->input('search') ?? '' }}">
                    </div>
                    <div class="col">
                        <button class="btn btn-primary w-100"><i class="fa fa-search"></i></button>
                    </div>
                    <div class="col">
                        <a href="{{ url()->current() }}" class="btn btn-secondary w-100">{{ translate('Reset') }}</a>
                    </div>
                </div>
            </form>
        </div>
        <div class="table-responsive">
            <table class="table sortable-table w-100">
                <thead>
                    <tr>
                        <th><i class="fa-solid fa-hashtag"></i></th>
                        <th>{{ translate('Details') }}</th>
                        <th>{{ translate('Position') }}</th>
                        <th class="text-end">{{ translate('Action') }}</th>
                    </tr>
                </thead>
                <tbody class="sortable-table-tbody">
                    @forelse ($currencies as $currency)
                        <tr data-id="{{ $currency->id }}">
                            <td>
                                <span class="sortable-table-handle me-2 text-muted">
                                    <i class="fas fa-arrows-alt fa-lg"></i>
                                </span>
                            </td>
                            <td>
                                <a href="{{ route('admin.financial.currencies.edit', $currency->id) }}">
                                    <img src="{{ $currency->getIconLink() }}" alt="{{ $currency->code }}" width="25px">
                                    <span class="text-dark ms-2">{{ $currency->code }} ({{ $currency->symbol }})</span>
                                </a>
                                @if ($currency->isDefault())
                                    <span class="badge bg-primary ms-2">{{ translate('Default') }}</span>
                                @endif
                            </td>
                            <td>{{ $currency->getPositionName() }}</td>
                            <td>
                                <div class="text-end">
                                    <button type="button" class="btn btn-sm rounded-3" data-bs-toggle="dropdown"
                                        aria-expanded="true">
                                        <i class="fa fa-ellipsis-v fa-sm text-muted"></i>
                                    </button>
                                    <ul class="dropdown-menu dropdown-menu-sm-end" data-popper-placement="bottom-end">
                                        <li>
                                            <a class="dropdown-item"
                                                href="{{ route('admin.financial.currencies.edit', $currency->id) }}"><i
                                                    class="fa-regular fa-pen-to-square me-2"></i>{{ translate('Edit') }}</a>
                                        </li>
                                        @if (!$currency->isDefault())
                                            <li>
                                                <hr class="dropdown-divider" />
                                            </li>
                                            <li>
                                                <form
                                                    action="{{ route('admin.financial.currencies.default', $currency->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    <button class="vironeer-form-confirm dropdown-item action-confirm">
                                                        <i class="far fa-bookmark me-2"></i>
                                                        {{ translate('Make default') }}
                                                    </button>
                                                </form>
                                            </li>
                                            <li>
                                                <hr class="dropdown-divider" />
                                            </li>
                                            <li>
                                                <form
                                                    action="{{ route('admin.financial.currencies.destroy', $currency->id) }}"
                                                    method="POST">
                                                    @csrf @method('DELETE')
                                                    <button class="action-confirm dropdown-item text-danger"><i
                                                            class="far fa-trash-alt me-2"></i>{{ translate('Delete') }}</button>
                                                </form>
                                            </li>
                                        @endif
                                    </ul>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="text-center">
                                <span class="text-muted">{{ translate('No Currencies Found') }}</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>
    @push('top_scripts')
        <script>
            const sortingRoute = "{{ route('admin.financial.currencies.sortable') }}";
        </script>
    @endpush
    @push('styles_libs')
        <link rel="stylesheet" href="{{ asset('vendor/libs/bootstrap/select/bootstrap-select.min.css') }}">
        <link href="{{ asset('vendor/libs/jquery/jquery-ui.min.css') }}" />
    @endpush
    @push('scripts_libs')
        <script src="{{ asset('vendor/libs/bootstrap/select/bootstrap-select.min.js') }}"></script>
        <script src="{{ asset('vendor/libs/jquery/jquery-ui.min.js') }}"></script>
    @endpush
@endsection
