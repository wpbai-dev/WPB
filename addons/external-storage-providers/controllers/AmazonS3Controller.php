...
class AmazonS3Controller extends Controller
{
    ...
    {
        setEnv('AWS_ACCESS_KEY_ID', $credentials->access_key_id);
        setEnv('AWS_SECRET_ACCESS_KEY', $credentials->secret_access_key);
        setEnv('AWS_DEFAULT_REGION', $credentials->default_region);
        setEnv('AWS_BUCKET', $credentials->bucket);
        setEnv('AWS_URL', $credentials->url);
        setEnv('AWS_ENDPOINT', $credentials->endpoint);
    }
