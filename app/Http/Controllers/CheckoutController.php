...
class CheckoutController extends Controller
{
    public function index($id)
    {
        ...
        $paymentGateways = PaymentGateway::excludeWalletIfZero(authUser()->balance)
            ->active()->count();
    ...
    private function trxHasAChange($trx)
    {
        if ($trx->isTypePurchase()) {
            $totalItemsAmount = 0;
            $trxItems = $trx->trxItems()->whereHas('item', function ($query) {
                $query->active();
            })->get();

            foreach ($trxItems as $trxItem) {
                if ($trxItem->isLicenseTypeExtended()) {
                    if ($trxItem->item->hasExtendedLicense()) {
